...
contract StargateWithdrawTest is StargateBaseTest {
    ...
    function testDepositAndWithdrawSeveralTime() public {
        _deposit(address(goblinBank), SMALL_AMOUNT / 5);
        _deposit(address(goblinBank), SMALL_AMOUNT / 5);
        _deposit(address(goblinBank), SMALL_AMOUNT / 5);
        _deposit(address(goblinBank), SMALL_AMOUNT / 5);
        _deposit(address(goblinBank), SMALL_AMOUNT / 5);
        ...
        (uint256 instantAmount0, ) = _withdraw(address(goblinBank), shareFraction, ALICE);  // 0.5 total
        (uint256 instantAmount1, ) = _withdraw(address(goblinBank), shareFraction, ALICE);  // 0.25 total  (50% of what was left)
        (uint256 instantAmount2, ) = _withdraw(address(goblinBank), shareFraction, ALICE);  // 0.125 total (50% of what was left)
        (uint256 instantAmount3, ) = _withdraw(address(goblinBank), _calculateTotalShareFraction(), ALICE);  // all the rest
        ...
        assertEq(IERC20(stargateYieldModule.baseToken()).balanceOf(address(goblinBank)), 0);
    ...
    function testOnlyGoblinBankCanWithdraw() public {
        ...
        _withdraw(address(goblinBank), _calculateTotalShareFraction(), ALICE);
    ...
    function testCannotWithdrawAsyncIfMsgValueIsLessThanExecutionFee() public {
        ...
        deal(address(goblinBank), executionFee);
        vm.prank(address(goblinBank));
    ...
    function _canWithdraw(uint256 _deltaCredit) internal returns (uint256 instantAmount, uint256 pendingAmount) {
        ...
        (uint256 instantAmount, uint256 pendingAmount) = _withdraw(address(goblinBank), shareFraction, ALICE);
    ...
    function _canWithdrawAll(uint256 _deltaCredit) internal returns (uint256 instantAmount, uint256 pendingAmount) {
        ...
        _deposit(address(goblinBank), BIG_AMOUNT);
        StargateYieldModuleDeltaCreditMock(payable(stargateYieldModule)).setPoolDeltaCredit(deltaCredit);
        (uint256 instantAmount, uint256 pendingAmount) = _withdraw(address(goblinBank), shareFractionAmount, ALICE);
