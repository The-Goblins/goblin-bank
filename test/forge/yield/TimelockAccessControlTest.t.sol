...
contract TimelockAccessControlTest is Deployer, TestHelper {

    address[][] public dummyRoutes = [[ARB, COMP, WETH, USDC]];
    ...
    function testOnlyProposerCanSchedule() public {
        ...
        timelock.schedule(address(uniV3), 0, abi.encodeWithSelector(UniV3DexModule.setRoutes.selector, routes), bytes32(0), 0, TIMELOCK_MIN_DELAY);
        vm.stopPrank();
        ...
        vm.startPrank(OWNER);
        bytes32 id = timelock.hashOperation(address(uniV3), 0, abi.encodeWithSelector(UniV3DexModule.setRoutes.selector, routes), bytes32(0), 0);
        timelock.schedule(address(uniV3), 0, abi.encodeWithSelector(UniV3DexModule.setRoutes.selector, routes), bytes32(0), 0, TIMELOCK_MIN_DELAY);
        vm.stopPrank();
    ...
    function testOnlyExecutorCanExecute() public {
        ...
        timelock.execute(address(uniV3), 0, abi.encodeWithSelector(UniV3DexModule.setRoutes.selector, routes), bytes32(0), 0);
        vm.stopPrank();
    ...
    function testCanGrantATimelockRoleThroughTheTimelock() public {
        vm.startPrank(OWNER);
        bytes32 id = timelock.hashOperation(address(uniV3), 0, abi.encodeWithSelector(AccessControl.grantRole.selector, keccak256("EXECUTOR_ROLE"), RANDOM_ADDRESS), bytes32(0), 0);
    ...
    function testCanRevokeATimelockRoleThroughTheTimelock() public {
        ...
        vm.startPrank(OWNER);
        bytes32 id = timelock.hashOperation(address(uniV3), 0, abi.encodeWithSelector(AccessControl.revokeRole.selector, keccak256("EXECUTOR_ROLE"), RANDOM_ADDRESS), bytes32(0), 0);
