...
abstract contract BaseModuleUpgradedMock is IYieldModule, UUPSUpgradeable, OwnableUpgradeable, Rescuable {
    ...
    modifier onlySmartFarmooor() {
        require(msg.sender == goblinBank, "BaseModule: only Goblin Bank");
        _;
    }
     ...
     * @param   _goblinBank Goblin Bank of the module
     ...
     * @param   _goblinBank  Address of the Goblin bank
     */
    function _setGoblinBank(address _goblinBank) private {
        require(
            _goblinBank != address(0),
            "BaseModule: cannot be the zero address"
        );
        goblinBank = _goblinBank;
    }
