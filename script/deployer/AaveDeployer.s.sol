...
contract AaveDeployer is DataLoader, AddressFetcher {
    ...
    function verifyAaveYieldModule(address owner, address smartFarmooor, address dex) internal {
        ...
        assertEq(aaveYieldModule.goblinBank(), smartFarmooor);
        ...
        assertEq(aaveYieldModule.rewards(0), ARB);
        assertEq(aaveYieldModule.name(), AAVE_YIELD_MODULE_NAME);
        assertGt(
            IERC20(ARB).allowance(address(aaveYieldModule), dex),
            type(uint256).max / 2
        );
    ...
    function printAaveStorage() internal view {
        ...
        console.log("Goblin bank:        ", aaveYieldModule.goblinBank());
        console.log("baseToken:          ", aaveYieldModule.baseToken());
